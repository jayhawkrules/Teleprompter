...
a duplicate — add a brief comment with the new context instead.

**Recurring-completed guard.** Open-task dedup misses an item whose *prior* instance was
already completed, so it would recreate it. That's correct for a genuinely new
occurrence — but if an **identical** task was **completed within the meeting's cadence
window** (≤8 days for a weekly sync, ≤32 for monthly), treat it as the same recurrence:
comment-and-skip rather than spawn a duplicate. When cadence is unclear, **create** — a
redundant task is cheaper than a dropped one.
