...
(new file or enrichment, deduping against existing); then **clear the file back to its
header**. This is the only path cloud-found facts reach real memory — don't skip it.
...
halves never double-route the same meeting. Skip any meeting id already in `routed`;
after routing, add it to the Drive manifest. (A local
`~/Documents/Claude/Projects/.meeting-router/manifest.json` may exist as a cache, but
Drive wins on conflict.)
